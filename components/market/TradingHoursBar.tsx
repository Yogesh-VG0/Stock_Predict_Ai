...
const TradingHoursPopup = dynamic(() => import("./TradingHoursPopup"), { ssr: false });
...
export function TradingHoursBar({ compact = false, marketClosed = false, holidayName }: TradingHoursBarProps) {
  ...
  if (marketClosed) {
    return (
      <>
        <div onClick={() => setPopupOpen(true)} className={`cursor-pointer ${compact ? "w-full flex flex-col items-center mb-0 relative" : "w-full flex flex-col items-center mb-4 relative"}`} style={{ overflow: 'visible' }}>
        ...
        <TradingHoursPopup open={popupOpen} onClose={() => setPopupOpen(false)} />
      </>
    );
  }
